...
let wakeLock = null; // Declare a variable to store the wake lock object
...
async function requestWakeLock() {
  ...
  if ('wakeLock' in navigator) {
    try {
      ...
      // The screen won't dim while the wake lock is active
    } catch (err) {
...
// Example of using the wake lock:
document.getElementById('start-recording').addEventListener('click', () => {
  // Request the wake lock when starting recording
  requestWakeLock();
...
document.getElementById('stop-recording').addEventListener('click', () => {
  ...
  // Release the wake lock when recording is stopped
  releaseWakeLock();
});
